...
Public Sub ckFormAndSave(nSave As Boolean, Optional cWB As Workbook, Optional wbJustOpened As Boolean = False)
    ...
    Call ckformating() :
    If nSave Then execWBbackupSave()
...
With curWS
    ...
    stOldColRow = oldColRow.Row
        ...
        If Not ckThis.NumberFormat = "General" Or
            Not ckThis.NumberFormatLocal = "General" Then

                .Activate
                MsgBox prompt:="This workbook is showing an issue with formating." &
                            vbNewLine & "Please check for formatting errors within the workbook." &
                            vbNewLine & vbNewLine &
                            "If this problem persists, please contact your system administrator for help.",
...
Private Sub execWBbackupSave()
    ...
    Call saveQuoteData("\\SVFS02\cfmInfo\cfm Documents\Amos Quote Program\In " &
        "Progress and Old Quote Data", 666, "SYSTEM", Format(curDateTime, "MMM-d-yyyy"), Format(curDateTime, " h N S AMPM"),
...
Private Function delOrNew() As Boolean
    ...
    If pSessionSaves = 1 And Not pubWBjustOpened Then
        ...
        res = MsgBox(prompt:="Reminder: Sessions reset each time the workbook opens:" &
                            vbNewLine & Space(5) &
                            "You have already saved once this session, " &
                            "would you like to delete your last saved backup copy for this session?" &
                            vbNewLine & vbNewLine & "Note: Your new save has already completed successfully",
